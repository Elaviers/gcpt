...
private:
	...
	virtual Texture* _CreateResource(const Array<byte>& data, const String& name, const String& extension) override;
	virtual void _ResourceRead(Texture& texture, const Array<byte>& data, const String& extension) override;

	byte _maxAnisotropy;
	byte _maxMipLevels;

public:
	TextureManager() : AssetManager({".txt", ".ttex", ".png", ".tex"}), _maxAnisotropy(0), _maxMipLevels(0) {}
