...
//#define LABEL_IMGUI_DEBUG_PATH "imgui.h"
...
void UILabel::UpdateBounds()
{
	...
	if (_font)
	{
		...
		float fontSize = _textSize >= 0.f ? _textSize : availableH;
	...
	void UILabel::Update(float deltaTime)
	{
		...
		ImGui::Begin(_text.ToString().begin());
		...
		_transform.SetRotation(Vector3(0.f, 0.f, rot));
	}
#else
void UILabel::Update(float deltaTime) {}
#endif
