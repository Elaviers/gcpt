...
public:
	...
	void ResetMouseMove() { OnMouseMove(false, INFINITY, INFINITY, 0); }
	...
	virtual bool OverlapsPoint(float x, float y, uint32 hoverId) const
	{
		if (hoverId > 0)
			return hoverId == _id;
	...
	virtual void RenderID(RenderQueue&) const;
	...
	virtual bool OnMouseMove(bool blocked, float x, float y, uint32 hoverId);
	...
	bool SetCursorPos(float x, float y, uint32 hoverId) { return OnMouseMove(false, x, y, hoverId); }
};
