...
FontTTF::FontTTF() : _face(nullptr), _size(0), _descender(0)
{
#ifdef BUILD_CLIENT
	_vao = _vbo = 0;
#endif
...
void FontTTF::_CMD_LoadFont(const Array<String>& args)
{
	if (args.GetSize() > 0)
	{
		FontManager* const fontManager = g_engine.fonts;
		const FT_Library ftLib = g_engine.freetype;
