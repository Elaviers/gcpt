...
public:
	...
	constexpr size_type IndexOf(const CharType* string, size_type start = 0) const
	{
	...
	constexpr size_type LastIndexOf(const CharType* string, size_type start) const
	{
		if (start < _length)
		{
			const size_t len = StringUtils::Length(string);
	...
	constexpr size_type IndexOfAny(const CharType* possibleChars, size_type start = 0) const
	{
	...
	constexpr size_type LastIndexOfAny(const CharType* possibleChars, size_type start) const
	{
	...
	constexpr size_type LastIndexOf(const CharType* string) const { return LastIndexOf(string, _length - 1); }
	constexpr size_type LastIndexOfAny(const CharType* possibleChars) const { return LastIndexOfAny(possibleChars, _length - 1); }

	constexpr TString Replace(const CharType* src, const CharType* with) const
	{
		size_type end = IndexOf(src);
	...
	constexpr TString& Lower() noexcept
	{
	...
	static constexpr TString Concat(T first, ARGS&&... args) { return From(first) + Concat(std::forward<ARGS>(args)...); }
