...
void FontTexture::_CMD_texture(const Array<String>& args)
{
#ifdef BUILD_CLIENT
	if (args.GetSize() > 0)
	{
		_texture = g_engine.textures->Get(args[0]);
	}
#endif
}

void FontTexture::_CMD_region(const Array<String>& args)
{
	if (args.GetSize() >= 6)
	{
		if (args[0] == "space")
...
void FontTexture::ReadText(const String & string)
{
