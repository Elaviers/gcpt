...
String PropertyCollection::HandleCommand(void* obj, const Array<String>& tokens) const
{
	if (tokens.GetSize() > 0)
	{
		...
		if (property)
		{
			...
			return property->HandleCommand(obj, args);
		}
	}
...
void PropertyCollection::Transfer(const void* from, void* to) const
{
	for (size_t i = 0; i < _properties.GetSize(); ++i)
		if ((_properties[i]->GetFlags() & PropertyFlags::READONLY) == 0)
			_properties[i]->TransferTo(from, to);
}
