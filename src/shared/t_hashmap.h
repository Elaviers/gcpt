...
private:
	struct VNode
	{
		...
		VNode(const K& key, Args&&... vargs) : kv{ key, std::forward<Args>(vargs)... }, next(nullptr), iNext(nullptr) {}
	};
	...
	constexpr void _EnforceLoad()
	{
		...
		{
			//TODO --- hashmap --- Grow to prime sizes to minimize hash collisions!
...
public:
	...
	constexpr Hashmap(const std::initializer_list<Pair<const K, V>>& pairs, const HasherType& hasher = HasherType(), const AllocatorType& allocator = AllocatorType()) noexcept :
		Hashmap(hasher, allocator)
	{
		for (const auto& pair : pairs)
			GetOrDefaultEmplace(pair.first, pair.second);
	}
